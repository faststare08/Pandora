...
def foutInlog():
    ...
    msg.setWindowIcon(QIcon('./images/logos/logo.jpg')) 
    msg.setIcon(QMessageBox.Warning)
    msg.setText('Inloggegevens onjuist!')
    msg.setWindowTitle('AANMELDING')
    msg.exec_()
...
def hoofdMenu(m_email):
    ...
    class Widget(QDialog):
        def __init__(self, parent=None):
            ...
            k12Edit.addItem('2. Bestellijst - Orderbrieven printen')  
            k12Edit.addItem('3. Grafiek voorraden \n    magazijn financiëel') 
            k12Edit.addItem('4. Overzicht voorraden financiëel')
            k12Edit.activated[str].connect(self.k12Changed)
            
            self.Keuze13 = QLabel()
            k13Edit = QComboBox()
            k13Edit.setFixedWidth(280)
    ...
    if mk0 == '1' and mp[0][4] == '1':
        ...
        while True:
            ...
            while check:
                if mregel == 0:
                     minkgeg = invoerInkooporder.Inkooporder(m_email)
                mregel += 1
                check = invoerInkooporder.inkoopRegels(minkgeg,mregel)
                if check == 2:
                    mregel -= 1
        ...
        regel = 0
        while True:
            wijzInkooporder.inkoopRegels(regel, m_email)
        ...
        while True:
            ...
            while check:
                if mregel == 0:
                     minkgeg = invoerDienstenorder.Inkooporder(m_email)
                mregel += 1
                check = invoerDienstenorder.inkoopRegels(minkgeg,mregel)
    elif mk3 == '4' and mp[3][4] == '1':
        import wijzDienstenorder
        regel = 0
        while True:
            wijzDienstenorder.dienstenRegels(regel, m_email)
        ...
        voorraadbeheersing.jaarVerbruik(m_email)
    elif mk12 == '2' and mp[12][1] == '1':
        import artikelbestellingen
        artikelbestellingen.toonArtikellijst(m_email)
